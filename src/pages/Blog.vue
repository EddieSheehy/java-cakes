...
<div class="mb-3 right">
    <button type="button" @click="postComment" class="btn btn-primary">Post Comment</button></div>
<div class="mb-3 right">
...
export default {
    ...
    methods: {
        ...
        postComment(){
            ...
            let uid = "anonymous";
            if(auth.currentUser != null) // Check that there is a logged in user
            {
                uid = auth.currentUser.uid; // if logged in then assign uid
            }
            postComment({"handle": this.handle, "comment": this.comment, "uid": uid}).then((result) => {
